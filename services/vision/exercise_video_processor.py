...
class VideoProcessorClass(VideoProcessorBase):
    ...
    def _draw_form_score(self, img, score):
        """Camera frame pe form score dikhao."""
        h, w = img.shape[:2]
        ...
        cv2.rectangle(img, (w - 200, 10), (w - 10, 60), (0, 0, 0), -1)
        cv2.putText(
            img,
            f"FORM: {score}/100",
            (w - 195, 48),
    ...
    def recv(self, frame):
        ...
        if result.pose_landmarks:
            ...
            if detector:
                ...
                # Form score calculate karo
                form_score, form_feedbacks = get_form_score(ex_type, landmarks)
                metrics["form_score"]     = form_score
                metrics["form_feedbacks"] = form_feedbacks

                self._draw_overlays(image, metrics, ex_type)
                self._draw_form_score(image, form_score)
